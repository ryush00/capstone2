      ...
      limit.times do |i|
        ...
        Post.transaction do
          ...
          if post.present?
            ...
            Rails.logger.info "게시글을 불러오지 못해 스킵합니다."
          end
        end
    ...
    begin
      ...
      if response.status == 200
        ...
        if post.save
          ...
          # 게시글 내용이 성공적으로 업데이트되면 AI 분석 작업을 큐에 추가
          if post.content.present? && post.ai_analyzed_at.blank?
            ExtractDataJob.perform_later(post.id)
            Rails.logger.info "AI 분석 작업을 큐에 추가했습니다: 게시글 ID #{post.id}"
          end
        else
